...
static int _fd_flush(IO *io) { return 0; }

static int _fd_close(IO *io);

static const IO_METHOD _fd_method = {IO_POSIX, _fd_read,  _fd_write, _fd_seek,
                                     _fd_tell, _fd_flush, _fd_close};
...
static int64_t _fd_read(IO *io, void *buffer, int64_t size)
{
	...
	if (_fd_read_chunked(size)) {
		...
		do {
			...
			ret = _fd_read_fn(io->handle, b, to_read);
			if (ret < 0) {
				read_length = ret;
				break;
			}
...
static int64_t _fd_write(IO *io, const void *buffer, int64_t size)
{
	...
	if (_fd_write_chunked(size)) {
		...
		do {
			...
			ret = _fd_write_fn(io->handle, b, to_write);
			if (ret < 0) {
				write_length = ret;
				break;
			}
