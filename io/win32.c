...
static const IO_METHOD _w32_method = {
    IO_WIN32,  _w32_read,  _w32_write, _w32_seek,
    _w32_tell, _w32_flush, _w32_close,
};
...
static int64_t _w32_read(IO *io, void *buffer, int64_t size)
{
	...
	if (size > INT32_MAX) {
		...
		do {
			...
			result = ReadFile(hFile, b, to_read, &n_read, NULL);
			if (!result)
				return -1;
			if (n_read == 0)
				break;
...
static int64_t _w32_write(IO *io, const void *buffer, int64_t size)
{
	...
	if (size > INT32_MAX) {
		...
		do {
			...
			result = WriteFile(hFile, b, to_write, &n_write, NULL);
			if (!result)
				return -1;
			if (n_write == 0)
				break;
...
static int64_t _w32_seek(IO *io, int64_t offset, int whence)
{
	...
	case IO_SEEK_SET:
		new_offset_low =
		    SetFilePointer(hFile, offset_low, &offset_high, FILE_BEGIN);
		break;
	case IO_SEEK_CUR:
		new_offset_low = SetFilePointer(hFile, offset_low, &offset_high,
		                                FILE_CURRENT);
		break;
	case IO_SEEK_END:
		new_offset_low =
		    SetFilePointer(hFile, offset_low, &offset_high, FILE_END);
		break;
	}
	...
	if (offset_high == 0) {
		...
		new_offset =
		    (uint32_t)new_offset_low | ((uint64_t)(offset_high) << 32);
	}

	return new_offset;
}

static int64_t _w32_tell(IO *io) { return _w32_seek(io, 0, IO_SEEK_CUR); }

static int _w32_flush(IO *io) { return 0; }
...
IO *IO_create_handle(void *hFile, int should_close) { return NULL; }
